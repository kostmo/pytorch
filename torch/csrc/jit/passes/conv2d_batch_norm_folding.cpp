...
void transformAndCopyConvWeightAndBias(
  ...
  std::unique_ptr<AliasDb> aliasDb =
      torch::make_unique<AliasDb>(graph_ptr);
  ...
  auto new_weight_val = *(tryInsertConstant(graph, std::get<0>(new_weight_bias)));
...
void foldBatchNorm2dInConv2d(std::shared_ptr<Graph>& graph) {
  ...
  for (const auto& match : matches) {
    ...
    Value* matched_bn_running_mean = match.values_map.at(pattern_bn_running_mean);
    ...
    TORCH_CHECK(!(toIValue(matched_training)->toBool()),
...
void FoldConvBatchNorm2d(std::shared_ptr<Graph>& graph) {
  ConstantPooling(graph);
  ConstantPropagation(graph);
  graph_rewrite_helper::replaceConvolutionWithConv2d(graph);
  foldBatchNorm2dInConv2d(graph);
  LOG(INFO) << "Finished FoldConv2dBatchNorm pass. If you did not get"
